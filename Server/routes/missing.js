...
router.post("/", protect, async (req, res) => {
  try {
    const { name, age, gender, lastSeen, location, description, contactInfo } =
      req.body;
...
router.put("/:id", protect, async (req, res) => {
  try {
    ...
    if (missingPerson.reportedBy.toString() !== req.user._id.toString()) {
      return res
        .status(401)
        .json({ message: "User not authorized to update this report" });
    }
...
router.delete("/:id", protect, async (req, res) => {
  try {
    ...
    if (missingPerson.reportedBy.toString() !== req.user._id.toString()) {
      return res
        .status(401)
        .json({ message: "User not authorized to delete this report" });
    }
